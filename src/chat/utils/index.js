...
export function getEmojiData() {
    const emotions = require('@/chat/assets/emotions.json')
    let groups = {},
        categories = [],
        map = {};
...
// 根据文件消息的文件后缀判断文件消息类型：图片、视频、录音
export function getFileType(data) {
    let url = data.file.file_url
    // 如果是自己发送的消息，会有custom代表文件类型
    if (data.custom) {
        return data.custom
    } else {
        let imgs = ['jpg', 'jpeg', 'gif', 'png']
        let videos = ['avi', 'mp4', 'wmv', 'mov']
        let voice = ['mp3', 'wav', 'aac', 'm4a', 'weba']
        let name = url.toLowerCase().split('.')
        let houzhui = name[name.length - 1]
        if (imgs.includes(houzhui)) return 'image'
        else if (videos.includes(houzhui)) return 'video'
        else if (voice.includes(houzhui)) return 'voice'
        else return 'noFound'
    }
}

// 格式化成能显示的富文本
export function initRichText(content) {
    let a = "" + content;
    let str = a.replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&amp;/g, "&").replace(/&quot;/g, '"')
        .replace(/&apos;/g, "'");
    return str
}
...
/**
 * @description 根据当前路由模式，获取地址栏拼接的指定参数
 * @param { String } key - 参数名 
 * @returns { String } 参数值
 */
export function getUrlParam(key, isHistory = true) {
    if (isHistory) {
        // history模式下获取参数
